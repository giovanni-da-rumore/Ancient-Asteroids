;(function() {
  ...
  Asteroid.prototype.asteroidCollision = function(otherObject) {
    ...
    var pos1 = [this.pos[0] + (vel1[0] * 3), this.pos[1] + (vel1[1] * 3)]
    var asteroid1 = new Asteroids.Asteroid(pos1, this.game, vel1, this.radius / 2)
    this.game.addObject(asteroid1)

    // vel2[0] = Math.cos(degrees2) * this.vel[0];
    // vel2[1] =  Math.sin(degrees2) * this.vel[1];
    // vel2 = randomVel(4);
    var pos2 = [this.pos[0] + (vel2[0] * 3), this.pos[1] + (vel2[1] * 3)]

    var asteroid2 = new Asteroids.Asteroid(pos2, this.game, vel2, this.radius / 2)
