;(function() {
  "use strict";
  if (typeof Asteroids === "undefined") {
    window.Asteroids = {};
  }

  var RADIUS = 50;
  var COLOR = "#388E8E";
  var DEGREES = 0;
  var VASEIMAGES = ["./images/alc_sappho.jpg", "./images/alc_sappho.jpg", "./images/alc_sappho.jpg",
                    "./images/mainad.jpg", "./images/ganymede.jpg", "./images/zeus.jpg", "./images/eros.jpg",
                    "./images/heracles.jpg", "./images/ganymede.jpg"]


  var Asteroid = Asteroids.Asteroid = function (pos, game, vel, radius, color) {
    var moHash = {};
    moHash["pos"] = pos;
    moHash["radius"] = radius || RADIUS;
    moHash["color"] = color || COLOR;
    moHash["game"] = game;
    moHash["vel"] = vel || randomVel(4);

    Asteroids.MovingObject.call(this, moHash);
  };

  var randomVel = function(length) {
    var min = 0.7
    var x = (Math.random() * (length - min)) + min;
    var y = (Math.random() * (length - min)) + min;
    return [x,y];
  };

  var offSetter = function () {
    return (Math.random() * (.5)) + .5

  }

  var fourAsteroids = function () {
    return Math.random() > 0.5
  }

  Asteroids.Util.inherits(Asteroid, Asteroids.MovingObject);


  Asteroid.prototype.draw = function (ctx) {

    // ctx.fillStyle = this.color;
    // ctx.beginPath();
    // ctx.arc(
    //   this.pos[0],
    //   this.pos[1],
    //   this.radius,
    //   0,
    //   2 * Math.PI,
    //   false
    // );
    // ctx.fill();
    //

    // tmpCtx.beginPath();
    // tmpCtx.arc(85, 85, 40, 0, Math.PI * 2, true);
    // tmpCrx.lineWidth(5)
    // tmpCtx.clip();
    // tmpCtx.closePath();
    //
    // var thumbImg = new Image();
    //
    //
    //
    // thumbImg.src = 'https://s-media-cache-ak0.pinimg.com/236x/2f/35/17/2f351759ebc0e2a981d9ef6ec1bf946c.jpg'


    // thumbImg.onload = function() {
      var degrees = Math.atan(this.vel[1], this.vel[0])
      var radius = this.radius
      ctx.save();
      ctx.translate(this.pos[0], this.pos[1])
      ctx.rotate(degrees);
      ctx.translate(-1 * this.pos[0], -1 * this.pos[1]);
      ctx.beginPath();
      ctx.arc(this.pos[0], this.pos[1] , radius, 0, Math.PI * 2, true);
      ctx.lineWidth = 8;
      ctx.strokeStyle = '#f28600';
      ctx.strokeStyle = '#f27a00';
      ctx.stroke();
      ctx.closePath();
      ctx.clip();
      ctx.drawImage(this.image, this.pos[0] - radius, this.pos[1] - radius, radius * 2, radius * 2);


      ctx.restore();

  // }.bind(this)


    //
    // var thumbImg = document.createElement('img')
    //
    // thumbImg.src = './images/monet.jpg';
    // thumbImg.onload = function() {
    //   ctx.save();
    //   ctx.beginPath();
    //   ctx.arc(this.pos[0], this.pos[1], this.radius, 0, Math.PI * 2, false);
    //   ctx.closePath();
    //   ctx.clip();
    //
    //   ctx.drawImage(thumbImg, this.pos[0] - this.radius, this.pos[1] + this.radius, this.radius * 2, this.radius * 2);
    //   debugger;
    //
    //   ctx.beginPath();
    //   ctx.arc(this.pos[0] - this.radius, this.pos[1] - this.radius, this.radius, 0, Math.PI * 2, false);
    //   ctx.clip();
    //   ctx.closePath();
    //   // ctx.restore();
    // }.bind(this);

  };

  Asteroid.prototype.collideWith = function(otherObject) {

    if (this.game.state != "playing") {
      return;
    }

    if (otherObject instanceof Asteroids.Ship) {
      this.game.lives -= 1;
      otherObject.relocate();


    } else if (otherObject instanceof Asteroids.Bullet) {
      this.asteroidCollision(otherObject);
      this.game.remove(otherObject);
      this.game.remove(this);
      this.game.score += 5;
    }
  }



  Asteroid.prototype.asteroidCollision = function(otherObject) {
    if (this.radius / 1.6 < 20 ) {
      return;
    }
    var vel1= randomVel(3);
    var vel2 = [(vel1[0] * -1) + offSetter(), (vel1[1] * -1) + offSetter()];
    var vel3 = [(vel1[0] * -1) + offSetter(), vel1[1] + offSetter()];
    this.addAsteroid(vel1);
    this.addAsteroid(vel2);
    this.addAsteroid(vel3);

    if (fourAsteroids()) {
      var vel4 = [(vel1[0]) + offSetter(), (vel1[1] * -1) + offSetter()];
      this.addAsteroid(vel4);
    }
  };


  Asteroid.prototype.addAsteroid = function (vel) {
    var radius = this.radius / 1.6
    var pos = [this.pos[0] + (vel[0] * 6), this.pos[1] + (vel[1] * 6)]
    var asteroid = new Asteroids.Asteroid(pos, this.game, vel, radius)
    asteroid.addImage();
    this.game.addObject(asteroid)

  };


  Asteroid.prototype.addImage = function (asteroid) {
    var index = Math.floor(Math.random() * VASEIMAGES.length)
    var img = new Image();
    img.src = VASEIMAGES[index];
    this.image = img;

  }



})();
